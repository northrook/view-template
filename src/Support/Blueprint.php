...
final class Blueprint
{
    ...
    {
        if ( ! \class_exists( '\Nette\PhpGenerator' ) ) {
            throw new CompileError(
                'The required class "Nette\PhpGenerator" does not exist',
            );
        }
    }
    ...
    public function generateTemplateClass(
        ...
        if ( ! \class_exists( Php\ClassType::class ) ) {
            throw new LogicException(
                'Nette PhpGenerator is required to generate blueprint, install package `nette/php-generator`.',
            );
        }
    ...
    public function addFunctions( Php\ClassType $class, array $funcs ) : void
    {
        foreach ( $funcs as $name => $func ) {
            ...
            $type   = $this->printType( $method->getReturnType(), $method->isReturnNullable(), $class->getNamespace() )
                    ?: 'mixed';
    ...
    public function printParameters(
        ...
        foreach ( $list as $param ) {
            ...
            $params[] = \ltrim( $this->printType( $param->getType(), $param->isNullable(), $namespace ).' ' )
                        ...
                        .( $variadic ? '...' : '' )
                        .'$'.$param->getName()
                        .( $param->hasDefaultValue() && ! $variadic ? ' = '.\var_export(
