...
use Core\View\Template\{Engine, Exception\RuntimeException};
...
final class Tracer
{
    ...
    {
        try {
            $e     = new RuntimeException( 'Your location in Latte templates' );
        ...
        catch ( Throwable $regexException ) {
        ...
        foreach ( $trace as $item ) {
            ...
            if ( $object instanceof Template ) {
                ...
                elseif ( $method === 'render' && $object->getReferenceType() ) {
                    ...
                    $res[] = [
                        'function' => '{'.$object->getReferenceType().' '.\basename(
                            $object->getName(),
                        ).'}',
                ...
                elseif ( $method === 'renderBlock' || $method === 'renderBlockParent' ) {
                    // {include block}
                    $res[\count( $res ) - 1]['args']
                            = self::filterParams( $item['args'][1] + $object->getParameters() );

                    if ( $method !== 'renderBlock' || isset( $item['args'][2] ) ) { // is not {block}
                        $res[] = [
                            'function' => '{include '.( $method === 'renderBlockParent' ? 'parent'
                                            : $item['args'][0] ).'}',
                            'file' => $object->getName(),
