...
final class TagLexer
{
    ...
    private const array KeywordsFollowed = [ // must follows ( & =
    ...
    private function tokenizeCode() : void
    {
        ...
        foreach ( $matches as $m ) {
            ...
            elseif ( isset( $m['Php_Identifier'] ) ) {
                ...
                $this->addToken(
                    self::Keywords[$lower] ?? ( \preg_match( '~[A-Z_][A-Z0-9_]{2,}$~DA', $m['Php_Identifier'] )
                        ? Token::Php_Constant : Token::Php_Identifier ),
                    $m['Php_Identifier'],
                );
            }
            elseif ( isset( $m['Php_IdentifierFollowed'] ) ) {
                ...
                $this->addToken(
                    self::KeywordsFollowed[$lower] ?? self::Keywords[$lower] ?? Token::Php_Identifier,
                    $m['Php_IdentifierFollowed'],
                );
            }
            ...
            elseif ( isset( $m['string'] ) ) {
                ...
                if (
                    ...
                    \array_splice(
                        ...
                        $count - 1,
                        null,
                        [new Token( Token::Php_ConstantEncapsedString, '"'.$token?->text.'"', $pos )],
                    );
                }
                goto matchRE;
            }
                ...
                elseif ( isset( $m['offset'] ) ) {
                    ...
                    elseif ( isset( $m['Php_NumString'] ) ) {
                        ...
                        throw new CompileException(
                            "Unexpected '".\substr( $this->input, $this->offset - 1, 5 )."'",
                            $this->position,
                        );
                    }
                    $this->addToken( null, ']' );
