...
final class PrintContext
{
    ...
    public function format( string $mask, mixed ...$args ) : string
    {
        ...
        $mask = \preg_replace_callback(
            ...
            function( $m ) use ( $args ) {
                [, $pos, $fn, $var] = $m;
                $var                = \substr( $var, 1, -1 );
                /** @var Nodes\Php\ModifierNode[] $args */
                return match ( $fn ) {
        ...
        return \preg_replace_callback(
            ...
            function( $m ) use ( $args ) {
                ...
                $code = match ( $format ) {
                    'dump' => PhpGenerator::dump( $arg ),
                    'node' => $arg ? $arg->print( $this ) : '',
                    'raw'  => (string) $arg,
                    'args' => $this->implode(
                        $arg instanceof Expression\ArrayNode
                                    ? $arg->toArguments() : $arg,
                    ),
    ...
    public function addBlock( Block $block ) : void
    {
        ...
        $block->method
                 = 'block'.\ucfirst( \trim( \preg_replace( '#\W+#', '_', $block->name->print( $this ) ), '_' ) );
    ...
    {
        return $node instanceof Nodes\Php\NameNode || $node instanceof Nodes\Php\IdentifierNode
                ? (string) $node
                : '{'.$node->print( $this ).'}';
    }
    ...
    {
        return $expr instanceof Nodes\Php\NameNode
    ...
    public function dereferenceExpr( Node $expr ) : string
    {
        return $expr instanceof Expression\VariableNode
               || $expr instanceof Nodes\Php\NameNode
               || $expr instanceof Expression\ArrayAccessNode
    ...
    /**
     * @param Nodes\Php\ArgumentNode[] $args
     */
    public function argumentsAsArray( array $args ) : string
    {
        $items = \array_map(
            fn( Nodes\Php\ArgumentNode $arg ) => $arg->toArrayItem(),
            $args,
        );
