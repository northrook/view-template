...
/**
 * Test: Core\View\Template\Runtime\Filters::escapeICal
 */
...
Assert::same('\"\;\\\\\,\:', Filters::escapeICal('";\,:'));
Assert::same('<br>', Filters::escapeICal(new Core\View\Template\Runtime\Html('<br>')));
Assert::same(
...
Assert::same(
	...
	Filters::escapeICal("\x10\x11\x12\x13\x14\x15\x16\x17\x18\x19\x1a\x1b\x1c\x1d\x1e\x1f"),
);

// invalid UTF-8
Assert::same("foo \u{D800} bar", Filters::escapeICal("foo \u{D800} bar")); // invalid codepoint high surrogates
Assert::same("foo \xE3\x80\\\x22 bar", Filters::escapeICal("foo \xE3\x80\x22 bar")); // stripped UTF
