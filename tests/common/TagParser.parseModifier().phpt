...
use Core\View\Template\Compiler\TagParser;
use Tester\Assert;
...
test('FilterInfo aware modifiers', function () {
	Assert::same('$this->filters->filterContent(\'mod\', $__filter__, @)', format('|mod', true));
	Assert::same('$this->filters->filterContent(\'escape\', $__filter__, $this->filters->filterContent(\'mod2\', $__filter__, $this->filters->filterContent(\'mod1\', $__filter__, @)))', format('|mod1|mod2|escape', true));
});
