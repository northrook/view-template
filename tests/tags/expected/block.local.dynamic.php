...
';
		foreach ($iterator = $__it__ = new Core\View\Template\Essential\CachingIterator(['dynamic', 'static'], $__it__ ?? null) as $name) /* line %d% */ {
			$this->addBlock($__name__ = $name, 'html', [[$this, 'blockName']], 'local');
		...
		echo "\n";
		$this->addBlock($__name__ = "word{$name}", 'html', [[$this, 'blockWord_name']], 'local');
		$this->renderBlock($__name__, get_defined_vars());
		echo '

';
		$this->addBlock($__name__ = "strip{$name}", 'html', [[$this, 'blockStrip_name']], 'local');
